    ...
    if ($statusUpload == 0) {
        $message = '<script>alert("' . $message . ',  Gambar tidak dapat diUpload");
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {                
                $query = mysqli_query($conn, "UPDATE tb_kegiatan SET foto='" . $kode_rand . $_FILES['foto']['name'] . "', nama_kegiatan = '$nama_kegiatan', narasumber = '$narasumber', keterangan = '$keterangan' WHERE id='$id'");
                if ($query) {
