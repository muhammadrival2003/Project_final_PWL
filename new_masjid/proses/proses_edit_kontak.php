...
if (!empty($_POST['edit_kontak_validate'])) {
    ...
    if ($statusUpload == 0) {
        $message = '<script>alert("' . $message . ',  Gambar tidak dapat diUpload");
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {                
                $query = mysqli_query($conn, "UPDATE tb_kontak SET foto='" . $kode_rand . $_FILES['foto']['name'] . "', nama = '$nama', jabatan = '$jabatan', nohp = '$nohp' WHERE id_kontak='$id'");
                if ($query) {
                    $message = '<script>alert("Kontak Berhasil diganti");
                window.location="../kelolaKontak"</script>';
                } else {
                    $message = '<script>alert("Kontak gagal dimasukkan");
                window.location="../kelolaKontak"</script>';
                }
            } else {
